...
{
public:
	Picture(cv::InputArray image, const cv::Rect &rect): cv::Mat(image.getMat()), enableAnimationTickCount(INT64_MAX), animation(new IdleAnimation()), frame(rect){}
	Picture(const CvArr* image, const cv::Rect &rect): cv::Mat(cv::cvarrToMat(image)), enableAnimationTickCount(INT64_MAX), animation(new IdleAnimation()), frame(rect){}
	...
	*/
	void setAnimation(const AnimationEnum &animationEnum, 
		cv::InputArray startImg, cv::InputArray endImg);
	...
	*/
	void setContent(cv::InputArray image);
	...
	*  \brief A setter for frame.
		...
		note: the location is the upper left of this picture.
	*/
	void setFrame(cv::InputArray roi, const cv::Rect &rect);
