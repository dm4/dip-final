...
void Picture::setContent(InputArray image)
{
...
void Picture::setFrame(cv::InputArray roi, const cv::Rect &rect)
{
    cerr << "Setting Picture frame" << endl;
	Mat origin;
	this->copyTo(origin);
	...
	setContent(origin);
	frame = rect;
    cerr << "Setting Picture frame" << " done" << endl;
}
