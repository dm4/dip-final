...
// unsigned receiveOSC(void *ArgList);
...
void hideWinBarAndMoveMouse()
{
	...
	cvSetWindowProperty(winName.c_str(), CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);
#else
	namedWindow(winName);
	cvMoveWindow(winName.c_str(), winPos.x, winPos.y);
...
// 	int event = MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE;
